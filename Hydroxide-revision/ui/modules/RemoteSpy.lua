...
local decompileCallingScriptContext = ContextMenuButton.new("rbxassetid://4666593447", "Copy Decompiled Origin Script")
...
local replicateSignalContext = ContextMenuButton.new("rbxassetid://4907151581", "Replicate to Server")
local fireSignalContext = ContextMenuButton.new("rbxassetid://4909102841", "Fire Signal")
local viewConnectionsContext = ContextMenuButton.new("rbxassetid://4891633802", "View Connections")
local viewSignalArgsContext = ContextMenuButton.new("rbxassetid://4702850565", "View Signal Arguments")
...
local remoteLogsMenu = ContextMenu.new({ scriptContext, callingScriptContext, decompileCallingScriptContext, spyClosureContext, repeatCallContext, fireSignalContext, replicateSignalContext, viewConnectionsContext, viewSignalArgsContext, viewAsHexContext })
...
decompileCallingScriptContext:SetCallback(function()
    if not selected.callingScript then
        MessageBox.Show("Error", "No calling script found for this remote call.", MessageType.OK)
        return
    end
    
    local oldStatus = oh.getStatus()
    oh.setStatus("Decompiling " .. selected.callingScript.Name .. "...")
    
    task.spawn(function()
        if decompile then
            local success, source = pcall(decompile, selected.callingScript)
            if success and source then
                if source:find("Decompilation failed") or source:find("Failed to decompile") or #source < 50 then
                    setClipboard("-- Decompilation Failed\n-- This script may be protected or obfuscated\n\n-- Script: " .. selected.callingScript:GetFullName())
                    MessageBox.Show("Decompilation Failed", "This script appears to be protected or obfuscated.\nA placeholder was copied to clipboard.", MessageType.OK)
                else
                    setClipboard(source)
                    MessageBox.Show("Success", "Decompiled source of '" .. selected.callingScript.Name .. "' copied to clipboard!\n\nPaste it in a text editor to view.", MessageType.OK)
                end
            else
                setClipboard("-- Decompilation Error: " .. tostring(source) .. "\n-- Script: " .. selected.callingScript:GetFullName())
                MessageBox.Show("Error", "Decompilation error: " .. tostring(source), MessageType.OK)
            end
        else
            setClipboard("-- Decompiler not available\n-- Your executor needs a decompile() function")
            MessageBox.Show("Error", "Your executor doesn't have a decompile() function.", MessageType.OK)
        end
...
replicateSignalContext:SetCallback(function()
    if not replicatesignal then
        MessageBox.Show("Not Supported", "Your executor doesn't support replicatesignal().\n\nThis is a Potassium-specific feature.", MessageType.OK)
        return
    end
    ...
    if remoteClassName ~= "RemoteEvent" and remoteClassName ~= "RemoteFunction" then
        MessageBox.Show("Error", "Can only replicate RemoteEvent or RemoteFunction signals.", MessageType.OK)
        return
    end
    
    local signalName = (remoteClassName == "RemoteEvent") and "OnServerEvent" or "OnServerInvoke"
    local signal = remoteInstance[signalName]
    
    if not cansignalreplicate or not cansignalreplicate(signal) then
        MessageBox.Show("Cannot Replicate", "This remote is not whitelisted for server replication.\n\nThe server will reject this signal.", MessageType.OK)
        return
    end
    
    local oldStatus = oh.getStatus()
    oh.setStatus("Replicating " .. remoteInstance.Name .. " to server...")
    
    local success, err = pcall(replicatesignal, signal, unpack(selected.args))
    
    if success then
        MessageBox.Show("Success", "Signal replicated to server!\n\nRemote: " .. remoteInstance.Name, MessageType.OK)
    else
        MessageBox.Show("Error", "Failed to replicate signal:\n\n" .. tostring(err), MessageType.OK)
    end
...
viewConnectionsContext:SetCallback(function()
    if not getconnections then
        MessageBox.Show("Not Supported", "Your executor doesn't support getconnections().", MessageType.OK)
        return
    end
    ...
    if #connections == 0 then
        MessageBox.Show("No Connections", "This remote has no connected listeners.", MessageType.OK)
        return
    end
    
    local connectionInfo = string.format("Remote: %s\nSignal: %s\nConnections: %d\n\n", remoteInstance.Name, signalName, #connections)
    
    for i, conn in pairs(connections) do
        connectionInfo = connectionInfo .. string.format("[%d] ", i)
        
        if conn.Function then
            local funcInfo = debug.getinfo(conn.Function)
            connectionInfo = connectionInfo .. string.format("Function: %s", funcInfo.name or "Anonymous")
            if funcInfo.source then
                connectionInfo = connectionInfo .. string.format("\n    Source: %s", funcInfo.source)
            end
        else
            connectionInfo = connectionInfo .. "Foreign State (C++ or CoreScript)"
        end
        
        if conn.Thread then
            connectionInfo = connectionInfo .. string.format("\n    Thread: %s", tostring(conn.Thread))
        end
        
        if typeof(conn.Script) == "Instance" then
            connectionInfo = connectionInfo .. string.format("\n    Script: %s", conn.Script:GetFullName())
        end
        
        connectionInfo = connectionInfo .. string.format("\n    Enabled: %s", tostring(conn.Enabled))
        connectionInfo = connectionInfo .. "\n\n"
    end
    
    setClipboard(connectionInfo)
    MessageBox.Show("Connections Info", connectionInfo:sub(1, 500) .. "\n\n[Full info copied to clipboard]", MessageType.OK)
end)

fireSignalContext:SetCallback(function()
    if not firesignal then
        MessageBox.Show("Not Supported", "Your executor doesn't support firesignal().\n\nThis is a Potassium-specific feature.", MessageType.OK)
        return
    end
    ...
    local oldStatus = oh.getStatus()
    oh.setStatus("Firing signal on " .. remoteInstance.Name)
    
    local success, err = pcall(firesignal, signal, unpack(selected.args))
    
    if success then
        MessageBox.Show("Success", "Signal fired locally!\n\nAll connections received the event.", MessageType.OK)
    else
        MessageBox.Show("Error", "Failed to fire signal:\n\n" .. tostring(err), MessageType.OK)
    end
...
viewSignalArgsContext:SetCallback(function()
    local remoteInstance = selected.remoteLog.Remote.Instance
    local remoteClassName = remoteInstance.ClassName
    ...
    local signal = remoteInstance[signalName]
    local argsInfo = string.format("Remote: %s\nSignal: %s\n\n", remoteInstance.Name, signalName)
    
    if getsignalarguments then
        local success, args = pcall(getsignalarguments, signal)
        if success and args then
            argsInfo = argsInfo .. "Expected Arguments:\n"
            for i, argType in pairs(args) do
                argsInfo = argsInfo .. string.format("  [%d] %s\n", i, argType)
            end
            argsInfo = argsInfo .. "\n"
        end
    end
    ...
    if getsignalargumentsinfo then
        local success, argsInfoData = pcall(getsignalargumentsinfo, signal)
        if success and argsInfoData then
            argsInfo = argsInfo .. "Detailed Argument Info:\n"
            for i, info in pairs(argsInfoData) do
                argsInfo = argsInfo .. string.format("  [%d] %s\n", i, tostring(info))
            end
            argsInfo = argsInfo .. "\n"
        end
    ...
    if cansignalreplicate then
        ...
        if success then
            argsInfo = argsInfo .. string.format("Can Replicate to Server: %s\n", tostring(canReplicate))
        end
    end
    
    argsInfo = argsInfo .. "\nCurrent Call Arguments:\n"
    for i, arg in pairs(selected.args) do
        argsInfo = argsInfo .. string.format("  [%d] (%s) %s\n", i, typeof(arg), tostring(arg):sub(1, 50))
    end
    
    setClipboard(argsInfo)
    MessageBox.Show("Signal Arguments", argsInfo:sub(1, 600) .. "\n\n[Full info copied to clipboard]", MessageType.OK)
end)
